...
async function buildContextAndAnalyze(
  ...
  if (!hasAiCredentials()) {
    return {
      ok: false,
      error:
        "AI認証情報が未設定です。AI_GATEWAY_API_KEY (推奨) または ANTHROPIC_API_KEY を .env に追加してください。",
    };
  }
  ...
  try {
    ...
    if (err instanceof EdinetApiError) {
      if (err.status === 404) {
      ...
      if (err.status === 429) {
        return {
          ok: false,
          error:
            "EDINET DB API のレート制限に達しました。時間をおいて再試行してください。",
        };
      }
      return {
        ok: false,
        error: `EDINET DB API エラー (${err.status}): ${err.body.slice(0, 200)}`,
      };
    }
    return {
      ok: false,
...
// unstable_cache は関数の「戻り値」をキャッシュするため、エラーオブジェクトを
// 返してしまうと 7日間エラー状態が焼き付く。
// そこで失敗時は throw して unstable_cache に保存させず、外側でキャッチして
// AnalysisError に変換する。
const NON_CACHEABLE_PREFIX = "__ANALYSIS_ERROR__:";

const cachedAnalyze = unstable_cache(
  async (edinetCode: string): Promise<AnalysisResult> => {
    const result = await buildContextAndAnalyze(edinetCode);
    if (!result.ok) {
      // throw → unstable_cache は結果を保存しない
      throw new Error(`${NON_CACHEABLE_PREFIX}${result.error}`);
    }
    return result;
  },
  ["company-analysis-v2"], // v1 時代の汚染エントリを無効化するためバンプ
...
export async function getCompanyAnalysis(
  ...
  try {
    return await cachedAnalyze(edinetCode);
  } catch (err) {
    if (err instanceof Error && err.message.startsWith(NON_CACHEABLE_PREFIX)) {
      return {
        ok: false,
        error: err.message.slice(NON_CACHEABLE_PREFIX.length),
      };
    }
    return {
      ok: false,
